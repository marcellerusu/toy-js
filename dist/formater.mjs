...
class Formatter {
  ...
  get is_in_if() {
    return (
      this.parents.at(-1) === IfBranch ||
      this.parents.at(-1) === ElseIfBranch ||
      this.parents.at(-1) === ElseBranch
    );
  }
  ...
  format_let_arr_deconstruction({ entries, rhs }) {
    return (
      "let [" +
      entries.map(this.format_let_arr_entry.bind(this)).join(", ") +
      "] = " +
      this.format_node(rhs)
    );
  }
  ...
  format_let_object_deconstruction({ entries, rhs }) {
    return (
      "let { " +
      entries.map(this.format_let_object_entry.bind(this)).join(", ") +
      " } = " +
      this.format_node(rhs)
    );
  }
  ...
  format_obj_lit({ entries }) {
    ...
    if (entries_js.map((x) => x.length).sum() > 35) {
      ...
      let obj_lit =
        ...
        this.padding +
        entries
          .map(this.format_obj_lit_entry.bind(this))
          .join(",\n" + this.padding) +
        ",\n";
      this.dedent();
  ...
  is_early_return(branches) {
    return (
      branches.length === 1 &&
      branches[0] instanceof IfBranch &&
      this.is_one_line_return(branches[0].body)
    );
  }
  format_if_statement({ branches }) {
    if (this.is_early_return(branches)) {
      ...
      return (
        branches.map(this.format_branch.bind(this)).join(this.padding) +
        this.padding +
        "end"
      );
    }
  ...
  format_class_arg(arg) {
    if (arg instanceof NamedClassArg) {
      ...
      return (
        "{ " +
        arg.entries.map(this.format_class_obj_arg.bind(this)).join(", ") +
        " }"
      );
    } else {
  ...
  format_class_def({ name, properties, entries }) {
    ...
    if (properties) {
      c +=
        "(" + properties.map(this.format_class_arg.bind(this)).join(", ") + ")";
    }
    c += "\n";
    this.indent();
    c +=
      this.padding +
      entries
        .map(this.format_class_entry.bind(this))
        .join("\n\n" + this.padding);
    this.dedent();
    c += "\n";
    return (c += "end");
  ...
  format_function_call({ lhs_expr, args }) {
    ...
    let args_output =
      start + args.map(this.format_node.bind(this)).join(separator);
    args_output += ending;
  ...
  format_for_loop({ iter_name, iterable_expr, body }) {
    let f =
      ...
      iter_name +
      " of " +
      this.format_node(iterable_expr) +
      " do\n";
  ...
  format_import_statement({ imports, path }) {
    ...
    let i =
      "import {" + space + imports.join("," + space) + "," + space[0] + "}";
    i += " from " + '"' + path + '"';
    return i;
  }
  ...
  format_array_literal({ elements }) {
    ...
    return (
      "[" +
      space +
      elements.map(this.format_node.bind(this)).join("," + space) +
      "," +
      space[0] +
      "]"
    );
  ...
  format_body(body, parent, indent_by = 2) {
    return (
      new Formatter(body, {
        ...
        parents: this.parents.concat(parent),
      })
        .format()
        .trimEnd() + "\n"
    );
  ...
  format_function_def({ name, args, body }) {
    if (
      body.length > 1 ||
      body[0] instanceof IfStatement ||
      this.too_long_for_one_liner(body[0])
    ) {
      let f = "def " + name;
      if (args.length > 0) {
      ...
      return (
        ...
        name +
        "(" +
        args.map(this.format_arg.bind(this)).join(", ") +
        ") = " +
        this.format_node(body[0], true)
      );
    }
  }
}
